...
const submitTest = async (req, res) => {
  try {
    ...
    const score = gradeQuizExams(
      userAnswers,
      exam.answer,
      exam.module,
      exam.subject,
      examResult?.totalNumberOfCorrectAnswers || {
        "TIẾNG ANH": { "MODULE 1": 0, "MODULE 2": 0 },
        TOÁN: { "MODULE 1": 0, "MODULE 2": 0 },
      }
    );

    const module1 = score[`${exam.subject}`]?.["MODULE 1"] || 0;
    const module2 = score[`${exam.subject}`]?.["MODULE 2"] || 0;
    const maxScore = exam.module === "MODULE 2-DIFFICULT" ? 800 : 700;
    let totalScore =
      ((module1 + module2) / assessment.totalQuestion) * maxScore;
    ...
    if (!examResult) {
      examResult = new ExamResultModel({
        ...
        totalNumberOfCorrectAnswers: score,
        modules: {
      ...
      return res.status(CONSTANT.OK).json({
        ...
        isDifficulty: module1 / exam.numberOfQuestions >= 0.6,
      });
    }
    ...
    if (exam.subject === "TIẾNG ANH" && exam.module === "MODULE 1") {
      const currentCacheTotalScore = examResult?.totalScore || {
        "TIẾNG ANH": 0,
        TOÁN: 0,
      };
      updateData.cacheTotalScore = currentCacheTotalScore;
    }
