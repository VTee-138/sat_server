...
const generateFilteredExam = async (req, res) => {
  try {
    ...
    for (let i = 0; i < filters.length; i++) {
      ...
      if (
        !questionType ||
        !CONSTANT.LIST_QUESTION_TYPE.includes(questionType)
      ) {
        return res.status(CONSTANT.BAD_REQUEST).json({
          message: `Filter ${i + 1}: questionType không hợp lệ`,
        });
      }
    ...
    for (let i = 0; i < validatedFilters.length; i++) {
      ...
      const query = {
        subject,
        "questionType.code": filter.questionType,
      ...
      // Thêm thông tin filter vào mỗi câu hỏi
      const questionsWithFilter = questions.map((q) => ({
        ...q,
        filterIndex: i + 1,
        filterInfo: {
          questionType: filter.questionType,
          difficulty: filter.difficulty,
          requestedCount: filter.numberOfQuestions,
        },
      }));

      allQuestions.push(...questionsWithFilter);

      availabilityChecks.push({
        filterIndex: i + 1,
        questionType: filter.questionType,
        difficulty: filter.difficulty,
        requested: filter.numberOfQuestions,
        available: available,
        obtained: questions.length,
      });
    }
    ...
    const examData = {
      ...
      subject,
      filterSummary: availabilityChecks,
